...
<!-- The example row's hash is the real digest of its own four fields against
     https://github.com/neonbjb/tortoise-tts, so it does not match this repository's URL - it is an
...
```bash
printf '%s' "ExampleModel v2:AI Corp:2026-08-12T14:30:00Z:https://github.com/neonbjb/tortoise-tts" | shasum -a 256
```

```python
import hashlib
hashlib.sha256("ExampleModel v2:AI Corp:2026-08-12T14:30:00Z:https://github.com/neonbjb/tortoise-tts".encode()).hexdigest()
```
...
2. **There.** In the credits your product already publishes — credits screen, about page,
   ...
   ```
   Includes material from neonbjb/tortoise-tts — https://github.com/neonbjb/tortoise-tts
